...
void main()
{
    ...
    // Berechne die Pixel-Position in Screen-Space
    vec2 pixelPos = v_TexCoord * u_Resolution;

    // Berechne die Distanz zur Maus
    float dist = distance(pixelPos, u_MousePos);

    // Berechne den Grauskalierungsfaktor (0 = farbig, 1 = grau)
    float grayFactor = smoothstep(u_Radius - 10.0, u_Radius, dist);

    // Berechne Grauskalenwert (Luminanz)
    float gray = dot(texColor.rgb, vec3(0.299, 0.587, 0.114));
    vec3 grayColor = vec3(gray);

    // Mische zwischen Farbe und Graustufen
    ...
    // Outline-Farbe (weiß, kannst du anpassen)
...
struct ViewportTestApp : Sketch
{
    ...
    RenderBuffer rb = createRenderBuffer(wallpaper.getSize().x, wallpaper.getSize().y);
